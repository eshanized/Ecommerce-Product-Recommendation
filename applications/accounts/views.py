...
def signin(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect("applications.accounts:signin")
...
def signup(request):
    if request.method == 'POST':
        ...
        return redirect("applications.accounts:profileset")
...
def profileset(request):
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            return redirect("applications.accounts:shippinginfo")
    else:
...
def acprofile(request):
    ...
    if not request.user.is_staff:
        ...
        if request.method == 'POST':
            ...
            return redirect("applications.accounts:address")
    else:
...
def orderdetail(request, orderid):
    ...
    if request.user.is_authenticated and Profile.objects.filter(user=request.user).exists():
            ...
            if request.user.profile != order.cart.profile:
                return redirect("applications.accounts:orders")
    else:
        return redirect("applications.accounts:signin")
